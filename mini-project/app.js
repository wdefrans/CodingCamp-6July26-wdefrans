...
function renderChart() {
  ...
  const expenseTxns = getExpenseTransactions();
...
function handleFormSubmit(e) {
  ...
  elDesc.classList.toggle("error", !!errors.description);
  elAmount.classList.toggle("error", !!errors.amount);
  ...
  showToast(
    `${selectedType === "income" ? "Income" : "Expense"} added ✓`
  );
}
...
function openLimitModal() {
  ...
  openModal(elLimitModal);
  setTimeout(() => elLimitInput.focus(), 100);
}

function closeLimitModal() {
    closeModal(elLimitModal);
}
